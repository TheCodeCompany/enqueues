...
- **FEATURE**: Asset caching with admin toggles and a Settings → Enqueues page
  ...
  - **Build-time asset manifest** (opt-in) — `wp enqueues manifest build` snapshots the theme template scan **and the per-block version map** into `{theme}/dist/enqueues-manifest.php`, so production requests read one opcache-cached file instead of walking the theme tree and stat-storming block assets. It supersedes the `theme_template_files` scan/transient and the block-version `filemtime` compute when present, is ignored in local dev, in the **Off** cache mode, and on a build-signature mismatch (stale → correct-but-slower, never stale output), is written atomically (temp + rename), and is fully opt-in (no file = prior behaviour). Filters `enqueues_manifest_enabled` / `enqueues_manifest_path` / `enqueues_manifest_validate_signature`; CLI `wp enqueues manifest build|clear|status`. See [docs/PERFORMANCE.md](docs/PERFORMANCE.md#build-time-manifest-deploy-generated-fastest).
